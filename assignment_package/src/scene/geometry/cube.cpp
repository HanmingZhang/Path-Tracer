...
float Cube::Area() const
{
    ...
    // since our original is a 1x1x1 Cube
...
//    tangent = glm::cross(GetCubeNormal(P), P);
//    bitangent = glm::cross(GetCubeNormal(P), tangent);


//    tangent = glm::vec3(glm::normalize(transform.T() * glm::vec4(tangent, 0.f)));
//    *tan = tangent;

//    bitangent = glm::vec3(glm::normalize(transform.T() * glm::vec4(bitangent, 0.f)));
//    *bit = bitangent;

    Vector3f n = GetCubeNormal(P);

    if (n.x == 1.0f || n.x == -1.0f){
        tangent = Vector3f(0.0f, 0.0f, 1.0f);
    }
