...
#include "microfacetbrdf.h"
 ...
 void MetalMaterial::ProduceBSDF(Intersection *isect) const {
    ...
    float uRough =
        uRoughness ? uRoughness : roughness;
    float vRough =
        vRoughness ? vRoughness : roughness;
    ...
    Fresnel* fresnel = new FresnelConductor(Color3f(1.f, 1.f, 1.f), eta, k);

    MicrofacetDistribution* distrib = new TrowbridgeReitzDistribution(uRough, vRough);


    isect->bsdf->Add(new MicrofacetBRDF(Color3f(1.f, 1.f, 1.f), distrib, fresnel));
}
