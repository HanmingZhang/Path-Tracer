...
public:
    ...
    Transform LoadTransform(QJsonObject &transform, bool isNoShape = false);
    glm::vec3 ToVec3(const QJsonArray &s);
    glm::vec3 ToVec3(const QStringRef &s);

    bool LoadEnvironment(QJsonObject &environment, QMap<QString, std::shared_ptr<Material>> mtl_map, const QStringRef &local_path, Scene &scene, QList<std::shared_ptr<Primitive> > *primitives, QList<std::shared_ptr<Light> > *lights);
