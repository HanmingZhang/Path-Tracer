...
public:
    Bounds3f()
        : min(Point3f(0.f)), max(Point3f(0.f))
    {}
    ...
    inline bool Inside(const Point3f &p) const{
    ...
    inline bool IntersectBoundingBox(const Point3f& b_min, const Point3f& b_max){

        float new_min_X = std::max(min.x, b_min.x);
        float new_min_Y = std::max(min.y, b_min.y);
        float new_min_Z = std::max(min.z, b_min.z);

        float new_max_X = std::min(max.x, b_max.x);
        float new_max_Y = std::min(max.y, b_max.y);
        float new_max_Z = std::min(max.z, b_max.z);
...
inline bool IntersectBoundingBox(const Bounds3f& b);
